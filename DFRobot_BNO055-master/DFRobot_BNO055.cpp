...
void DFRobot_BNO055::saveOffsets(unsigned int address)    //saves offset structure into eeprom at byte 100 upwards
{
...
bool DFRobot_BNO055::loadOffsets(unsigned int address) //loads offsets structure from eeprom at byte 100 upwards and loads the offsets onto the compass
{
    if(!_cached)
    {
...
void DFRobot_BNO055::getOffsets(struct calibOffsets *ptr)
{
    uint8_t tmp = 0;
    writePhase(eBNO055_REGISTER_ACC_OFFSET_X_LSB);
    Wire.requestFrom(BNO055_ADDRESS, 22, true);
...
void DFRobot_BNO055::setOffsets(struct calibOffsets *ptr)  //writes given offset structure into the compass
{
    writeByte(eBNO055_REGISTER_OPR_MODE, eCONFIGMODE);
    delay(19);
    ...
    writeByte(eBNO055_REGISTER_OPR_MODE, eNDOF);
    delay(7);
}

inline void DFRobot_BNO055::writePhase(uint8_t regaddr)	//Write Phase needed to tell from which address we want to read
{
	Wire.beginTransmission(BNO055_ADDRESS);
	Wire.write(regaddr);
	Wire.endTransmission();
